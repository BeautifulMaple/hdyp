...
public class Parking_Red extends AppCompatActivity {
    ...
    private static final String CARNUM_PARK_ID = null;

    private static List<SensorDataModel> sensorDataList = new ArrayList<>();
    private boolean isPark1Orange = false;//주황색인가?
    private boolean isPark2Orange = false;
...
//                hideCarNumText();
            ...
            // 주황색으로 변경하는 조건을 "No."가 아닌 경우로 변경
            if (!"No.".equals(parkingText.getText().toString())) {
                if("P01".equals(parkingText.getText().toString())) {
                    if (isPark2Orange) {
                        // P02가 이미 주황색이면 빨간색으로 변경
                        parkingImageView.setBackgroundColor(Color.RED);
                        isPark2Orange=false;
                    }
                    // 주황색으로 변경
                    parkingImageView.setBackgroundColor(Color.argb(255, 255, 165, 0));
                    isPark1Orange=true;
                    
                }else if("P02".equals(parkingText.getText().toString())){
                    if (isPark1Orange) {
                        // P02가 이미 주황색이면 빨간색으로 변경
                        parkingImageView.setBackgroundColor(Color.RED);
                        isPark1Orange=false;
...
//        // carnum_park TextView 업데이트
//        TextView carnumParkTextView = findViewById(R.id.carnum_park);
//        carnumParkTextView.setText(" ID: " + parkingText.getText().toString());
    }
...
//
        ...
        if(retTag.getTag() == "parking_green1" || retTag.getTag() == "parking_green2") {
            ...
            }else if(retTag.getTag() == parkingImageView2.getTag()){
        ...
        // carnum_park TextView 업데이트
//        TextView carnumParkTextView = findViewById(R.id.carnum_park);
//        carnumParkTextView.setText(" ID: " + carnum_park.toString());
    }
