...
public class Parking_garage extends AppCompatActivity {
...
//            showCarNumText();
    ...
    private void handleRefreshButton() {
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("park");
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        Boolean isOccupied = snapshot.child("is_occupied").getValue(Boolean.class);
                        Long timestampLong = snapshot.child("timestamp").getValue(Long.class);

                        if (isOccupied != null && timestampLong != null) {
                            String formattedTimestamp = formatDate(new Date(timestampLong));
                            SensorDataModel sensorData = new SensorDataModel(snapshot.getKey(), isOccupied, formattedTimestamp);
                            sensorDataList.add(sensorData);
                        } else {
                            Log.d("FirebaseData", "잘못된 데이터 형식");
                        }
                    }

                    // 새로고침된 데이터를 기반으로 주차 상태 UI를 업데이트합니다.
                    fetchParkingStatus(sensorDataList);
                    Toast.makeText(Parking_garage.this, "데이터가 성공적으로 갱신되었습니다.", Toast.LENGTH_SHORT).show();
                } else {
                    Log.d("Parking_Red", "데이터 갱신 실패: 데이터가 존재하지 않습니다.");
                    Toast.makeText(Parking_garage.this, "데이터 갱신에 실패했습니다.", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Log.d("Parking_Red", "데이터 갱신 실패", databaseError.toException());
                Toast.makeText(Parking_garage.this, "데이터 갱신에 실패했습니다.", Toast.LENGTH_SHORT).show();
            }
        });
    }
